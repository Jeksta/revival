...
-- Formspecs
--
function revival.get_bee_hive_active_formspec(life_percent)
	return "size[8,8.5]" ..
		"no_prepend[]" ..
			-- bg appearance
			"image[2,1;1,2.2;revival_bee_hive_progress_bg.png^" ..
				"[lowpart:" ..
					life_percent ..
						":revival_bee_hive_progress_fg.png]" ..
							"bgcolor[#343434;false]" ..
								"listcolors[#00000030;#F8F8FF30]" ..
									"background[0,0;8,4;revival_bee_hive_bg.png]" ..
										-- input lists
										"container[1,0.75]" ..
...
function revival.get_bee_hive_inactive_formspec()
	return "size[8,8.5]" ..
		"no_prepend[]" ..
			-- bg appearance
			"image[2,1;1,2.2;revival_bee_hive_progress_bg.png]" ..
...
local function add_mutation_to_drops(drops, properties, drone_species)
	...
	if 0.5 < math.random() then
		mutation = "revival:" .. mutation .. "_queen"
	else
...
local function hive_node_timer(pos, elapsed)
	...
	if hive_state == "inactive" and not inv:is_empty("queen") and not inv:is_empty("drone") then
		utils.swap_node(pos, "revival:bee_hive_active")

		hive_state = "active"

		local queen = inv:get_stack("queen", 1)
		local species = bee.get_species_from_name(queen:get_name())
		local properties = bee.get_species_properties(species)
	...
	if remaining_lifespan > 0 then
		...
		formspec = revival.get_bee_hive_active_formspec(100 - life_percent)
		...
		utils.swap_node(pos, "revival:bee_hive")
		hive_state = "inactive"
		life_percent = 0
		formspec = revival.get_bee_hive_inactive_formspec()
...
minetest.register_node(
	"revival:bee_hive",
	{
		...
		tiles = {
			"revival_bee_hive0_top.png",
			"revival_bee_hive0_top.png",
			"revival_bee_hive1_sides.png",
			"revival_bee_hive1_sides.png",
			"revival_bee_hive1_sides.png",
			"revival_bee_hive2_front.png"
		},
		paramtype2 = "facedir",
		drop = "revival:bee_hive",
		...
		on_construct = function(pos)
			local meta = minetest.get_meta(pos)
			meta:set_string("formspec", revival.get_bee_hive_inactive_formspec())
		...
		on_blast = function(pos)
			...
			drops[#drops + 1] = "revival:bee_hive"
